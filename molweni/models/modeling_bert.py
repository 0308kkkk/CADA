...
class BertForQuestionAnsweringCADA(BertPreTrainedModel):  
    def __init__(self, config):
        super().__init__(config)
        self.num_labels = config.num_labels
        ...
        self.spk_layers = nn.ModuleList([BertLayer(config) for _ in range(args.spk_layers)])
        self.utr_layers = nn.ModuleList([BertLayer(config) for _ in range(args.discourse_layers)])
        self.sigmoid = nn.Sigmoid()
        self.softmax = nn.Softmax(dim=-1)
        self.dropout = nn.Dropout(config.hidden_dropout_prob)
